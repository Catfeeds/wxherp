...
class Alipay {
    ...
    function __construct() {
        $this->alipay_config = require(__DIR__ . '/config.php');
    }
    ...
    function verifyNotify() {
        if (empty($_POST)) {//判断POST来的数组是否为空
            ...
            if ($this->debug) {
                ...
                $log_text = $log_text . self::createLinkString($_POST);
                self::logResult($log_text);
            }
            //验证
    ...
    function verifyReturn() {
        if (empty($_GET)) {//判断POST来的数组是否为空
            ...
            if ($this->debug) {
                ...
                $log_text = $log_text . self::createLinkString($_GET);
                self::logResult($log_text);
            }
            //验证
            //$responsetTxt的结果不是true，与服务器设置问题、合作身份者ID、notify_id一分钟失效有关
    ...
    function getSignVeryfy($para_temp, $sign) {
        ...
        switch (strtoupper(trim($this->alipay_config['sign_type']))) {
            case 'MD5' :
                $isSgin = self::md5Verify($prestr, $sign, $this->alipay_config['key']);
                break;
            default :
                $isSgin = false;
        }
    ...
    function getResponse($notify_id) {
        ...
        $responseTxt = self::getHttpResponseGET($veryfy_url, $this->alipay_config['cacert']);
