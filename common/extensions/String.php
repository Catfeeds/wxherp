...
class String {
     ...
     * @return string
     */
    public static function msubstr($str, $start = 0, $length = 255, $suffix = false, $charset = 'utf-8') {
        ...
        if (strlen($str) > $length - $start && $suffix) {
            return $slice . '...';
        }
        return $slice;
    }
    ...
    public static function randString($len = 6, $type = 0, $addChars = '') {
        ...
        if ($type != 6) {
            ...
            // 中文随机字
            for ($i = 0; $i < $len; $i++) {
                $str .= self::msubstr($chars, floor(mt_rand(0, mb_strlen($chars, 'utf-8') - 1)), 1, false, 'utf-8');
            }
        }
        return $str;
    }
